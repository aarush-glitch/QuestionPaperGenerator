...
st.session_state.setdefault("available_courses", ["Data Structures", "OOPs", "Software Engineering", "Computer Networks"])
...
def create_pdf_from_selection(selection, title: str = "Question Paper"):
    ...
    """
    try:
        # simple text layout — each question numbered with marks
        content_lines = [title, "", "Generated by CoE Project"]
        total_marks = 0
...
if st.session_state.wizard_step == 0:
    ...
    # Course selection (for future DB integration)
    ...
    with cols[0]:
        ...
        chosen = st.selectbox("Choose existing course or add new", options=course_options, index=0)
    with cols[1]:
        st.write("\n")
        st.write("\n")
    ...
    if not st.session_state.get("selected_course"):
        st.warning("Please choose or add a course before uploading or generating questions.")
